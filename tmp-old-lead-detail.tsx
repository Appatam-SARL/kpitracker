...
const LIFECYCLE_STAGES = [
  ...
  { key: 'QUALIFIED', label: 'Qualifi├®' },
  { key: 'LOST', label: 'Perdu' },
  { key: 'CONVERTED', label: 'Converti' },
] as const;

const ACTIVITY_TABS = [
  { key: 'activity', label: 'Activit├®', filterType: undefined },
...
/** R├®sum├® affich├® dans le bloc profil : pivots d'int├®r├¬t en priorit├®, sinon relations legacy. */
...
export default function LeadDetailPage({
    ...
    return (
      <DashboardShell>
          <div className='flex-1 flex flex-col gap-4 mt-2'>
            {/* Skeleton : bandeau D├®tails du lead + cycle de vie */}
  ...
  const handleLoadMoreActivities = async () => {
    if (!lead) return;
    // Onglet Agenda : pagination g├®r├®e par un autre composant
  ...
  return (
    <DashboardShell>
        {/* Section D├®tails du lead - Cycle de vie */}
          ...
          <div className='flex flex-col gap-3'>
            <div className='flex items-center justify-between'>
              ...
              <div className='flex items-center gap-2 text-[11px] text-gray-500'>
                <span>
                  ├ëtape du cycle : {STATUS_LABELS[lead.status] ?? lead.status}
                </span>
              ...
              <div className='flex flex-col items-center gap-3'>
                ...
                <div className='text-center'>
                  ...
                  <p className='text-[11px] text-gray-500'>
                    {lead.companyName ?? lead.company?.name ?? 'ÔÇö'}
                  </p>
                </div>
                <div className='flex items-center gap-2'>
                  <button
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      if (lead.status === 'CONVERTED') {
                        setConvertMessage({
                          type: 'error',
                          text: 'Ce prospect est d├®j├á enregistr├® comme client.',
                        });
                      ...
                      try {
                        ...
                        setConvertMessage({
                          type: 'success',
                          text: 'Prospect converti en client avec succ├¿s.',
                        });
                        setTimeout(() => setConvertMessage(null), 5000);
                        // Rafra├«chir les objectifs sur le dashboard (Mon objectif)
                ...
                {!hasPivotInterests && lead.status !== 'CONVERTED' && (
                  <p className='text-[10px] text-amber-700 bg-amber-50 border border-amber-200 px-3 py-2 rounded-xl'>
                    La conversion est impossible tant qu&apos;aucun int├®r├¬t n&apos;est
                    enregistr├® via le bloc{' '}
                    <a
              ...
              <div className='border-t border-gray-100 pt-4 space-y-2'>
                ...
                <div className='space-y-2 text-[11px]'>
                  <div className='flex justify-between'>
                    ...
                    <span className='text-gray-700'>{lead.email ?? 'ÔÇö'}</span>
                  </div>
                  <div className='flex justify-between'>
                    <span className='text-gray-500'>T├®l├®phone</span>
                    <span className='text-gray-700'>{lead.phone ?? 'ÔÇö'}</span>
                  </div>
                  <div className='flex justify-between'>
                  ...
                  <div className='flex justify-between'>
                    ...
                    <span className='text-gray-700'>{lead.jobTitle ?? 'ÔÇö'}</span>
                  </div>
                  <div className='flex justify-between'>
                  ...
                  <div className='flex justify-between'>
                    ...
                    <span className='text-gray-700'>{lead.source ?? 'ÔÇö'}</span>
                  </div>
                  ...
                  if (!res.ok) {
                    return {
                      ...
                      error:
                        typeof body.error === 'string'
                          ? body.error
                          : "Impossible d'enregistrer les int├®r├¬ts.",
                    };
                  }
            ...
            <NeumoCard className='p-4 flex flex-col gap-4 flex-1'>
              <div className='flex items-center gap-2 bg-gray-50 rounded-full px-3 py-1.5 border border-gray-100 text-xs w-full'>
                ...
                <input
                  ...
                  placeholder='Rechercher activit├®s, notes, emails...'
