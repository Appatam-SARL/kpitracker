...
const SLIDES: Slide[] = [
  {
    id: 'products',
    badge: 'Produits',
    title: 'Enrichissez votre catalogue',
    description:
      'Ajoutez les produits commercialisés pour les rattacher facilement à vos opportunités.',
    cta: 'Ajouter un produit',
    ctaHref: '#catalog-actions',
    tone: {
  ...
  },
  {
    id: 'services',
    badge: 'Services',
    title: 'Proposez vos prestations',
    description:
      'Référencez vos services pour standardiser l’offre et accélérer les propositions commerciales.',
  ...
  },
  {
    id: 'catalog',
    badge: 'Catalogue',
    title: 'Pilotez offre produits & services',
    description:
      'Basculez entre les onglets pour consulter, organiser et faire évoluer le catalogue de l’entreprise.',
    cta: 'Voir le catalogue',
    ctaHref: '#catalog-liste',
    tone: {
      panel: 'bg-slate-800',
      badge: 'bg-white/15',
      badgeText: 'text-white',
...
type ProductsServicesOnboardingCarouselProps = {
  onAddProduct?: () => void;
  onAddService?: () => void;
  onShowCatalog?: () => void;
};

export default function ProductsServicesOnboardingCarousel({
  onAddProduct,
  onAddService,
  onShowCatalog,
}: ProductsServicesOnboardingCarouselProps) {
  ...
  const handleCta = () => {
    if (slide.id === 'products') onAddProduct?.();
    if (slide.id === 'services') onAddService?.();
    if (slide.id === 'catalog') onShowCatalog?.();
      ...
      <AnimatePresence initial={false} custom={direction} mode='popLayout'>
        ...
        >
          ...
          <div className='pointer-events-none absolute inset-y-0 right-0 hidden w-[42%] sm:block'>
            ...
            {slide.id === 'catalog' && (
              <Plus className='absolute right-28 top-8 h-7 w-7 text-white/50' />
            )}
          </div>
