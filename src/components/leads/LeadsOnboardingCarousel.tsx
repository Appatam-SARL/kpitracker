...
const SLIDES: Slide[] = [
  ...
  },
  {
    id: 'views',
    badge: 'Pipeline',
    title: 'Liste, Kanban ou grille',
    description:
      'Passez d’une vue à l’autre pour piloter les négociations selon votre façon de travailler.',
    cta: 'Voir le pipeline',
    ctaHref: '#leads-pipeline',
    tone: {
      panel: 'bg-slate-800',
      badge: 'bg-white/15',
      badgeText: 'text-white',
...
type LeadsOnboardingCarouselProps = {
  onAddLead?: () => void;
  onImport?: () => void;
  onShowPipeline?: () => void;
};

export default function LeadsOnboardingCarousel({
  onAddLead,
  onImport,
  onShowPipeline,
}: LeadsOnboardingCarouselProps) {
  ...
  const handleCta = () => {
    if (slide.id === 'add') onAddLead?.();
    if (slide.id === 'import-export') onImport?.();
    if (slide.id === 'views') onShowPipeline?.();
      ...
      <AnimatePresence initial={false} custom={direction} mode='popLayout'>
        ...
        >
          ...
          <div className='pointer-events-none absolute inset-y-0 right-0 hidden w-[42%] sm:block'>
            ...
            {slide.id === 'import-export' && (
              <Download className='absolute right-28 top-8 h-7 w-7 text-white/50' />
            )}
          </div>
