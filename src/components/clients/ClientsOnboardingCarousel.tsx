...
const SLIDES: Slide[] = [
  {
    id: 'overview',
    badge: 'Clients',
    title: 'Vos clients gagnés',
    description:
      'Retrouvez ici les prospects convertis, distincts des leads encore en négociation.',
    cta: 'Voir la liste',
    ctaHref: '#clients-liste',
    tone: {
  ...
  },
  {
    ...
    description:
      'Filtrez par nom, contact ou société pour accéder rapidement à la bonne fiche client.',
    cta: 'Rechercher',
    ctaHref: '#clients-liste',
    tone: {
      panel: 'bg-sky-600',
  ...
  },
  {
    ...
    description:
      'Alternez les vues pour consulter, ouvrir le détail ou modifier une fiche client.',
    cta: 'Vue cartes',
    ctaHref: '#clients-liste',
    tone: {
      panel: 'bg-slate-800',
...
type ClientsOnboardingCarouselProps = {
  onShowList?: () => void;
  onFocusSearch?: () => void;
  onShowGrid?: () => void;
};

export default function ClientsOnboardingCarousel({
  onShowList,
  onFocusSearch,
  onShowGrid,
}: ClientsOnboardingCarouselProps) {
  ...
  const handleCta = () => {
    if (slide.id === 'overview') onShowList?.();
    if (slide.id === 'search') onFocusSearch?.();
    if (slide.id === 'views') onShowGrid?.();
      ...
      <AnimatePresence initial={false} custom={direction} mode='popLayout'>
        ...
        >
          ...
          <div className='pointer-events-none absolute inset-y-0 right-0 hidden w-[42%] sm:block'>
            ...
            {slide.id === 'views' && (
              <>
                <Eye className='absolute right-28 top-8 h-6 w-6 text-white/50' />
                <Pencil className='absolute right-32 bottom-10 h-5 w-5 text-white/40' />
              </>
            )}
          </div>
